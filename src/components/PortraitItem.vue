<template>
    <div class="portrait-item" v-loading="loading" element-loading-background="rgba(255, 255, 255, 1)">
        <div class="header">
            <div class="label">
                {{ template.tagVal }}
            </div>
            <div class="actions">
                <div class="chart-type-select">
                    ...
                    <!--                    </el-select>-->
                    <el-dropdown @command="changeType" size="small" trigger="click">
                        <span>{{ chartTypeLabel[chartType] }}<i class="el-icon-arrow-down el-icon--right"></i></span>
                        <el-dropdown-menu slot="dropdown">
                            <el-dropdown-item v-for="(type, index) of chartTypeMap" :key="`chart-type-idx-${index}`" :command="type.value">
                                {{ type.label }}
                            </el-dropdown-item>
                        </el-dropdown-menu>
                    </el-dropdown>
                ...
                <div class="chart-action">
                    <el-dropdown @command="handleCommand" size="small" trigger="click" v-if="indexType && !loading">
        ...
        <div class="content" v-else-if="template.id" :id="template.id + newId"></div>
        <div class="content" v-else :id="newId"></div>
        <el-dialog :close-on-click-modal="false" :visible.sync="showDataDetailDialog" width="600px" :append-to-body="true">
...
export default class PortraitItem extends Vue {
    ...
    private x: any = '：';
    private newId: string = this._getRandomString(12);
    private _getRandomString(len) {
        len = len || 32;
    ...
    private init() {
        let dom: any = null;
        if (this.template.id) {
            dom = document.getElementById(this.template.id + this.newId);
        } else {
            dom = document.getElementById(this.newId);
        }
        if (!dom) {
            return;
        }
    ...
    private loadData() {
        ...
        try {
            ...
            let style: string = 'COND_PROFILE';
            if (this.crowd.crowdType == 2) {
                style = 'FILE_PROFILE';
            }
            if (this.crowd.crowdType == 5) {
                style = 'COMB_PROFILE';
            }
            if (this.crowd.crowdType == 6) {
                style = 'GEO_PROFILE';
            }
            const data = {
                version: '5.0',
            ...
            this.autoSetChartType();
            this.loading = true;
            this.getTemplateData(JSON.stringify(data))
                .then((res: any) => {
                    this.data = res;
                    if (this.data && this.data.length > 0) {
                        ...
                        if (count < this.crowd.crowdRealNum) {
                            this.data.push({
                                count: this.crowd.crowdRealNum - count,
                                sort: 0,
                                tagVal: '',
                                tagValCn: '未知',
                            });
                    ...
                    // 当显示图例的时候，通过缓存tempData操作this.data
    ...
    private changeType(command: string) {
        this.chartType = command;
        this.$bus.$emit('chart-type', this.chartTypeValue[this.chartType], this.indexItem);
    }
    ...
    private buildLine() {
        const xData: any = [];
        const yData: any = [];
        const arrs: any = this.data.map(value => {
            return value.count;
        });
