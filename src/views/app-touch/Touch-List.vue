<template>
    <div class="touch-list">
        ...
        <div class="search-tool" style="padding-left: 0;">
            ...
            <div class="search-item" style="width: 35%;padding-right:0;">
                <el-select
                    v-model="searchParams.touchTimeType"
                    clearable
                    size="small"
                    placeholder="时间状态"
                    @change="changeTouchTimeType"
                    @clear="clearTouchTimeType"
                    style="width:30%;padding-right:5px;"
                >
                    <el-option v-for="item in [{value:1,label:'开始时间'},{value:2,label:'结束时间'}]" :key="item.value" :label="item.label" :value="item.value"></el-option>
                </el-select>
                <el-date-picker
                        v-model="searchParams.touchTime"
                        :disabled="searchParams.touchTimeType==undefined || searchParams.touchTimeType==null"
                        type="daterange"
                        align="right"
                        unlink-panels
                        size="small"
        ...
        <div class="touch-content">
                ...
                >
                    ...
                    <el-table-column prop="sysNames" label="应用系统" min-width="150"></el-table-column>
                    ...
                    <el-table-column label="操作" width="190" fixed="right">
                        <template slot-scope="scope">
                            <el-tooltip v-if="(scope.row.token == DEFAULT_TOKEN_JZT || scope.row.token == DEFAULT_TOKEN_5K) && scope.row.touchStatus == '4'" content="一次性任务不支持激活续期" placement="top" effect="light">
                                <span class="option-item disable">激活</span>
                            </el-tooltip>
                            <span v-else-if="scope.row.touchStatus == '4'" class="option-item" @click="showRestart(scope.row)">激活</span>
                            <span class="option-item" v-show="editStat(scope.row)" @click="showEdit(scope.row)">编辑</span>
                            <span class="option-item" v-show="delStat(scope.row)" @click="showDel(scope.row)">删除</span>
                            <span class="option-item" v-show="stopStat(scope.row)" @click="showStop(scope.row)">停止</span>
        ...
        <el-dialog :close-on-click-modal="false" :visible.sync="showActionDialog" width="500px">
                <div slot="title" class="dialog-title">激活应用任务</div>
                <div style="padding: 30px;">"激活"操作将执行自动续期从次日开始计算至此后第30天更新为应用周期结束日期，确认激活吗？</div>
                <span slot="footer" class="dialog-footer">
                    <el-button size="small" @click="showActionDialog = false">取 消</el-button>
                    <el-button size="small" type="primary" @click="actionAppTouch">确 定</el-button>
                </span>
            </el-dialog>
        <el-dialog :close-on-click-modal="false" :visible.sync="showEditDialog" width="600px">
            ...
            <el-form :model="newItem" :rules="addNewRules" ref="add-new" style="padding: 30px;">
                ...
                <el-form-item v-if="newItem.crowdUpdateCycle == 3" prop="crowdUpdateTime" ref="crowdUpdateTime" label="更新时间" label-width="120px">
                ...
                <el-form-item v-if="newItem.crowdUpdateCycle == 4" prop="crowdUpdateTime" ref="crowdUpdateTime" label="更新时间" label-width="120px">
                    <el-select
                ...
                <el-form-item prop="sysNames" ref="sysNames" label="应用系统" label-width="120px" v-if="!newItem.id">
                    <el-select
                        v-model="newItem.sysNames"
                        size="small"
                        style="width: 100%;"
                        filterable
                        remote
                    ...
                    >
                        <el-option v-for="item in sysList" :key="item.id" :label="item.sysName" :value="item.sysName"> </el-option>
                    </el-select>
                </el-form-item>
                <el-form-item prop="sysNames" label="应用系统" label-width="120px" v-else>
                    <el-select
                        v-model="newItem.sysNames"
                        size="small"
                        style="width: 100%;"
                ...
                <div class="info-item">
                    ...
                    <div class="info-item-body" v-if="newItem">
                        ...
                        <div class="info-item-property">
                            ...
                            <div class="info-item-property-value">{{ newItem.sysNames.length > 0 ? newItem.sysNames[0] : '-' }}</div>
                        </div>
                        <div class="info-item-property">
                ...
                <div class="crowd-export">
                    ...
                    >
                        <el-table-column label="开始时间" min-width="110">
                            <template slot-scope="scope">
                                <div v-if="scope.row.crowdExportStatus == 1 && (newItem.tokens.indexOf(DEFAULT_TOKEN_5K) != -1 || newItem.tokens.indexOf(DEFAULT_TOKEN_JZT) != -1)">--</div>
                        ...
                        <el-table-column label="目标详情" min-width="120">
                            <template slot-scope="scope">
                                <template v-if="newItem.tokens.indexOf(DEFAULT_TOKEN_5K) != -1">
                                    <template v-if="scope.row.crowdExportMsg">{{ `5k:${scope.row.crowdExportMsg}` }}</template>
                                </template>
                                <template v-if="newItem.tokens.indexOf(DEFAULT_TOKEN_JZT) != -1">
...
<script lang="ts">
    ...
    export default class TouchList extends Mixins(PageMixin) {
        ...
        private addNewRules: object = {
            ...
            sysNames: [{ type: 'array', required: true, message: '请选择应用系统', trigger: 'change' }],
        ...
        private selectableFun(val:any){
            if(val.tokens.indexOf(this.DEFAULT_TOKEN_5K) != -1 && val.crowdUpdateCycle == 1){
                return false;
            }
        ...
        get forzonTimeSelect() {
            if ((!this.newItem.crowdUpdateCycle || this.newItem.crowdUpdateCycle == 1) && this.newItem &&  this.newItem.tokens.indexOf(this.DEFAULT_TOKEN_5K) != -1) {
                return true;
            }
            if (this.newItem && this.newItem.tokens.indexOf(this.DEFAULT_TOKEN_JZT) != -1) {
                return true;
            }
            // if (this.newItem && (this.newItem.token == this.DEFAULT_TOKEN_JZT || this.newItem.token == this.DEFAULT_TOKEN_5K)) {
            //     return true;
            // }
            return false;
        }
        ...
        get disableRaido() {
            ...
            if (this.newItem.tokens.indexOf(this.DEFAULT_TOKEN_5K) != -1 || this.newItem.tokens.indexOf(this.DEFAULT_TOKEN_JZT) != -1) {
                return true;
            }
        ...
        get showPins() {
            ...
            if (this.newItem.tokens.indexOf(this.DEFAULT_TOKEN_JZT) != -1) {
                return true;
            }
            return false;
        }
        ...
        private changeTouchTimeType() {
            this.clearProperty(this.searchParams, 'touchTime')
            this.getAppTouchList();
        }
        private clearTouchTimeType() {
            this.clearProperty(this.searchParams, 'touchTimeType')
            this.clearProperty(this.searchParams, 'touchTime')
            this.getAppTouchList();
        }
        ...
        private setCrowd(crowd:any){
            ...
            this.$set(this.newItem, 'sysNames', []);
            this.$set(this.newItem, 'touchName', '');
            this.$set(this.newItem, 'tokens', []);

            this.$set(this.newItem, 'touchType', 1);
            if(crowd.crowdUpdateCycle){
            ...
            this.$refs['sysNames'].resetField();

            // setTimeout(()=>{
            //     this.$refs['add-new'].resetFields();
            // },50);
            // this.$refs['add-new'].resetFields();

            // const endTm: number = new Date(new Date(new Date().toLocaleDateString()).getTime() + 60 * 60 * 1000 * 24 * 30).getTime();
            // this.$set(this.newItem.touchTime, 1, endTm);
        ...
        private getAppTouchList() {
            ...
            this.queryList(params).then((res: any) => {
                ...
                this.dataList = res.list.map((item)=> {
                    if (item.sysNames == undefined || item.sysNames == null) {
                        item.sysNames = [item.sysName];
                    }
                    if (item.tokens == undefined || item.tokens == null) {
        ...
        private genQueryParam(params: any) {
            const query: any = {};
            if (params.sysNames && params.sysNames.length > 0) {
                query['sysNames'] = params.sysNames;
            }
            ...
            if (params.touchTime) {
                query['beginTime'] = params.touchTime[0];
                query['endTime'] = params.touchTime[1];
                if (params.touchTimeType != undefined || params.touchTimeType != null) {
                    query['touchTimeType'] = params.touchTimeType;
                }
            }
        ...
        private editStat(item: Touch) {
            if (item.sysNames.indexOf('京准通') != -1) {
                return false;
            }
        ...
        private showInfo(item: Touch) {
            ...
            if (this.newItem.sysNames == undefined || this.newItem.sysNames == null) {
                this.newItem.sysNames = [this.newItem.sysName];
            }
            if (this.newItem.tokens == undefined || this.newItem.tokens == null) {
                this.newItem.tokens = [this.newItem.token];
            }
            if (!this.newItem) {
                return;
            }
        ...
        private selectShowType(type: string) {
            ...
            if (type == "opration") {
                ...
                if (this.disableRaido) {
                    const crowdExportType = this.newItem.tokens.indexOf(this.DEFAULT_TOKEN_5K) != -1 ? 4 : 3;
                    this.crowdExport(
        ...
        private actionAppTouch() {
            this.showActionDialog = false;
            const data:any = {
                appTouchList:[
                    {
                        crowdCode: this.actionInfo.crowdCode,
            ...
            this.$api
                .post(`/api/ava/app/touch/activate`, data)
                .then(res => {
                    this.getAppTouchList();
                    this.delayDialog = false;
        ...
        private sysSelectChange(sysNames: string) {
            if (!this.newItem) {
                return;
            }
            if (sysNames.length > this.newItem.tokens.length) {
                // 增加
                const lastName = sysNames[sysNames.length - 1];
                const sys = this.sysList.find((item: any) => {
                    return item.sysName == lastName;
                });
                this.newItem.tokens.push(sys.token);
            } else {
                // 删除
            const syss = [];
            for (let i = 0; i < sysNames.length; i++) {
                const name = sysNames[i];
                const sys = this.sysList.find((item: any) => {
                    return item.sysName == name;
                });
                if (sys) {
                    syss.push(sys.token);
                } else {
                    this.newItem.sysNames.splice(i, 1);
                    return;
                }
            }
            this.newItem.tokens = syss;
            }
            // 5k和京准通的touchType = 3
            if (this.newItem.tokens.indexOf(this.DEFAULT_TOKEN_5K) != -1 || this.newItem.tokens.indexOf(this.DEFAULT_TOKEN_JZT) != -1) {
                if (this.newItem.tokens.length > 1) {
                    // 最后的不能选
                    this.$message.warning("不能同时选择5k或京准通与其他系统");
                    this.newItem.sysNames.splice(this.newItem.sysNames.length-1, 1);
                    this.newItem.tokens.splice(this.newItem.tokens.length-1, 1);
                    return;
                }
        ...
        private addOrupdateAppTouch() {
            (this.$refs['add-new'] as any).validate((valid: any) => {
                if (valid) {
                    ...
                    // 这里清理出5k和京准通和普通的系统，分别调用接口
                    // 5k和京准通 touchType = 3，普通不管
                    let type = '';
                    if (this.newItem.tokens.indexOf(this.DEFAULT_TOKEN_5K) != -1) {
                        type = '5k';
                        this.newItem.token = this.newItem.tokens[0];
                        this.newItem.sysName = this.newItem.sysNames[0];
                    } else if (this.newItem.tokens.indexOf(this.DEFAULT_TOKEN_JZT) != -1) {
        ...
        private crowdUpdateTimeSX(){
            this.$forceUpdate();
            // this.$refs['crowdUpdateTime'].resetField();
        }
        ...
        private genJZTTag(crowdExportMsg: any){
        ...
        private queryList(query?: any) {
            return new Promise((resolve, reject) => {
                const url = '/api/ava/app/touch/queryList';
                const data = {
                    pageNum: this.pageNo,
        ...
        private del(id: number) {
            return new Promise((resolve, reject) => {
                const url = '/api/ava/app/touch/del';
                const data = {
                    id: id
                };
                this.$api
        ...
        private add(params: any, type?: string) {
            let url: string = '/api/ava/app/touch/';
            if (type == '5k') {
                url = `${url}5k`;
            } else if (type == 'jzt') {
                url = `${url}jzt`;
                ...
                url = `${url}add`;
                const list = params.sysNames.map((item, i) => {
                    return {
                        sysName: item,
                        token: params.tokens[i]
                    }
                });
                params.sysTokenList = list;
                delete params.token;
        ...
        private stop(id: number) {
            return new Promise((resolve, reject) => {
                const url = '/api/ava/app/touch/stop';
                const data = {
                    id: id
                };
                this.$api
